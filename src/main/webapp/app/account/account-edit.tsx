...
export default function AccountEdit() {
  ...
  useDocumentTitle(t('account.edit.headline'));
  ...
  const prepareForm = async () => {
    try {
      ...
      const data = (await axios.get('/api/accounts/' + currentId)).data;
  ...
  const updateAccount = async (data: AccountDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.put('/api/accounts/' + currentId, data);
      navigate('/accounts', {
            state: {
              msgSuccess: t('account.update.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('account.edit.headline')}</h1>
      <div>
        <Link to="/accounts" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('account.edit.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(updateAccount)} noValidate>
      <InputRow useFormResult={useFormResult} object="account" field="id" disabled={true} type="number" />
      ...
      <input type="submit" value={t('account.edit.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 cursor-pointer mt-6" />
    </form>
  </>);
}
