...
export default function KycDocumentAdd() {
  const { t } = useTranslation();
  useDocumentTitle(t('kycDocument.add.headline'));
  ...
  const createKycDocument = async (data: KycDocumentDTO) => {
    window.scrollTo(0, 0);
    try {
      await axios.post('/api/kycDocuments', data);
      navigate('/kycDocuments', {
            state: {
              msgSuccess: t('kycDocument.create.success')
            }
          });
    } catch (error: any) {
  ...
  return (<>
    <div className="flex flex-wrap mb-6">
      <h1 className="grow text-3xl md:text-4xl font-medium mb-2">{t('kycDocument.add.headline')}</h1>
      <div>
        <Link to="/kycDocuments" className="inline-block text-white bg-gray-500 hover:bg-gray-600 focus:ring-gray-200 focus:ring-4 rounded px-5 py-2">{t('kycDocument.add.back')}</Link>
      </div>
    </div>
    <form onSubmit={useFormResult.handleSubmit(createKycDocument)} noValidate>
      ...
      <input type="submit" value={t('kycDocument.add.headline')} className="inline-block text-white bg-blue-600 hover:bg-blue-700 focus:ring-blue-300  focus:ring-4 rounded px-5 py-2 cursor-pointer mt-6" />
    </form>
  </>);
}
